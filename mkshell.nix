...
in with pkgs; # with import ./common/pythonlib.nix { python = python3; version = "python3.9"; };
stdenv.mkDerivation rec {
  ...
  buildInputs = [ mypy autoPatchelfHook ] ++ publibs ++ xlibs ++ qtlibs;
  ...
  ld = lib.makeLibraryPath ([ stdenv.cc.cc.lib fontconfig.lib ] ++ libs);
  ...
  strictDeps =
    true; # GCC fails because argument list created by nix is too long https://github.com/NixOS/nixpkgs/issues/41340
  LDFLAGS = "-L${glibc}/lib";
